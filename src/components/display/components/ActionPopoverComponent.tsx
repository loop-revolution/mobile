...
export const ActionPopoverComponent = ({ trigger, actions }: ActionPopoverArgs) => {
    ...
    let optionTitles = actions.map(({ text }: ActionPopoverAction) => (text))
    let optionIcons = actions.map(({ icon }: ActionPopoverAction) => (getComponentIcon(icon)))
    ...
    return (
        <>
            { trigger &&
                <TouchableRipple onPress={() => { showActionSheet() }} >
